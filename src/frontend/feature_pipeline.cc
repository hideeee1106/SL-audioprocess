...
void FeaturePipeline::AcceptWaveform(const std::vector<float>& wav) {
  ...
  int num_frames = fbank_.Compute(waves, &feats);
    ...
    if (context_expansion) {
        if (num_frames <= right_context) {
            throw std::runtime_error("Each chunk feature length must be larger than right context.");
        }

        std::vector<std::vector<float>> feats_pad;
        if (feature_remained.empty()) { // first chunk
            feats_pad.resize(num_frames + left_context);
            std::copy(feats.begin(), feats.end(), feats_pad.begin() + left_context);
            std::fill(feats_pad.begin(), feats_pad.begin() + left_context, feats.front());
        } else {
            feats_pad.resize(feature_remained.size() + num_frames);
            std::copy(feature_remained.begin(), feature_remained.end(), feats_pad.begin());
            std::copy(feats.begin(), feats.end(), feats_pad.begin() + feature_remained.size());
        }

        int ctx_frm = feats_pad.size() - (right_context + right_context);
        std::vector<std::vector<float>> feats_ctx(ctx_frm);

        for (int i = 0; i < ctx_frm; ++i) {
            std::vector<float>& frame_features = feats_ctx[i];
            for (int j = i; j < i + left_context + right_context + 1; ++j) {
                frame_features.insert(frame_features.end(), feats_pad[j].begin(), feats_pad[j].end());

            }
        }

        feature_remained = std::vector<std::vector<float>>(feats_pad.end() - (left_context + right_context), feats_pad.end());
        feats = std::move(feats_ctx);
    }

    if (downsampling > 1) {
        std::vector<std::vector<float>>feat_sample;
        int last_remainder = feats_ctx_offset==0 ?0:downsampling-feats_ctx_offset;
        int remainder = (feats.size() + last_remainder) % downsampling;
        for (size_t i = feats_ctx_offset; i < feats.size(); i += downsampling) {
            feat_sample.push_back(feats[i]);
        }
        feats_ctx_offset = remainder == 0 ? remainder : downsampling - remainder;
        feats=feat_sample;
    }
